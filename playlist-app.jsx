...
const DUMMY_SONGS = [
  { id:"1", title:"SELFISH (Feat. 슬기 Of Red Velvet)", artist:"문별(마마무)", youtubeId:"GNdCJGCWMSk", mood:["5월의 테마","청량","응원"], recommender:"SEONJU", comment:"통통 튀는 느낌의 비트! 자존감 상승시켜주는 노래.", date:"2026-05-11", month:"2026-05" },
  { id:"2", title:"Opening", artist:"LUCY", youtubeId:"O28bGr_6Seg", mood:["청량","설렘"], recommender:"모경", comment:"새로운 시작을 알리는 느낌! 봄에 딱 맞는 곡.", date:"2026-05-08", month:"2026-05" },
  { id:"3", title:"내 이름 맑음", artist:"QWER", youtubeId:"3lRPT2EYpHo", mood:["응원","5월의 테마"], recommender:"선주", comment:"내 이름처럼 맑게 살고 싶어지는 곡.", date:"2026-05-05", month:"2026-05" },
  { id:"4", title:"밤양갱", artist:"비비(BIBI)", youtubeId:"R3eTr9_7Mt0", mood:["감성","밤"], recommender:"숙", comment:"달달하면서 씁쓸한 묘한 중독성.", date:"2026-04-20", month:"2026-04" },
];
...
export default function PlaylistApp() {
  const [songs, setSongs] = useState(DUMMY_SONGS); // 더미 데이터 — Firebase 연결 후 []로 교체
  const [monthThemes, setMonthThemes] = useState({"2026-05":"5월의 테마","2026-04":"4월의 테마"});
  ...
  function handleAddSong() {
    ...
    setSongs(p=>[{id:Date.now().toString(),...form,youtubeId,date:today,month:today.slice(0,7)},...p]);
  ...
  function addInlineTag(){
    ...
    if(!moods.includes(tag)) setMoods(p=>[...p,tag]);
  ...
  function handleImportCSV() {
    ...
    if (Object.keys(themeMap).length > 0) {
      setMonthThemes(prev => ({ ...prev, ...themeMap }));
    }

    setSongs(p => [...imported, ...p]);
  ...
  function saveTheme() {
    ...
    setMonthThemes(p=>({...p,[themeForm.month]:themeForm.theme}));
  ...
  function addGlobalTag(){
    ...
    setMoods(p=>[...p,tag]); setNewTagInput(""); toast(`✅ "${tag}" 태그 추가됨!`);
  }
  ...
  function handleEditSave(){
    ...
    setSongs(p=>p.map(s=>s.id===editSong.id ? {...editSong, youtubeId} : s));
    setEditSong(null);
    toast("✅ 수정됐어요!");
  }
          ...
          <div style={s.mgmtBox}>
            ...
            {Object.entries(monthThemes).sort().reverse().map(([m,theme])=>(
              <div key={m} style={s.themeRow}>
                ...
                <button style={s.delBtn} onClick={()=>setMonthThemes(p=>{const n={...p};delete n[m];return n;})}>✕</button>
              </div>
            ))}
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:12}}>
              {moods.map(m=>(
                <div key={m} style={{display:"flex",alignItems:"center",gap:2}}>
                  ...
                  <button style={s.delBtn} onClick={()=>setMoods(p=>p.filter(x=>x!==m))}>✕</button>
                </div>
              ))}
            </div>
            <div style={{display:"flex",gap:8}}>
...
const s = {
  ...
  hero:{ background:C.bg2,border:`1px solid ${C.border}`,borderRadius:16,padding:"24px 20px 20px",margin:"18px 0 0",display:"flex",justifyContent:"space-between",alignItems:"flex-end" },
  heroLeft:{},
